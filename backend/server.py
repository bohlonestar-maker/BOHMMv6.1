...
async def start_discord_bot():
    ...
    try:
        sys.stderr.write("🔧 [DISCORD] Starting Discord activity tracking bot...\n")
        sys.stderr.flush()
        ...
        sys.stderr.write("✅ [DISCORD] Activity tracking bot started\n")
        sys.stderr.flush()
...
@api_router.post("/discord/link-member")
async def link_discord_member(discord_id: str, member_id: str, current_user: dict = Depends(verify_admin)):
    """Manually link a Discord member to a database member"""
    try:
        # Verify Discord member exists
        discord_member = await db.discord_members.find_one({"discord_id": discord_id}, {"_id": 0})
        if not discord_member:
            raise HTTPException(status_code=404, detail="Discord member not found")
        
        # Verify database member exists
        db_member = await db.members.find_one({"id": member_id}, {"_id": 0, "id": 1, "handle": 1, "name": 1})
        if not db_member:
        ...
        await log_activity(
            current_user['username'],
            "discord_link",
            f"Linked Discord member {discord_member.get('username')} to {db_member.get('handle')}"
        )
        
        return {
            "message": "Successfully linked Discord member",
            "discord_member": discord_member.get("username"),
            "database_member": db_member.get("handle")
        }
...
@api_router.delete("/discord/unlink-member/{discord_id}")
async def unlink_discord_member(discord_id: str, current_user: dict = Depends(verify_admin)):
    """Unlink a Discord member from a database member"""
    try:
        # Verify Discord member exists
        discord_member = await db.discord_members.find_one({"discord_id": discord_id}, {"_id": 0})
        if not discord_member:
        ...
        # Get current link info for logging
        linked_member_id = discord_member.get("member_id")
        if linked_member_id:
            db_member = await db.members.find_one({"id": linked_member_id}, {"_id": 0, "handle": 1})
            linked_handle = db_member.get("handle") if db_member else "Unknown"
        else:
            linked_handle = "None"
        ...
        await log_activity(
            current_user['username'],
            "discord_unlink",
            f"Unlinked Discord member {discord_member.get('username')} from {linked_handle}"
        )
        
        return {
            "message": "Successfully unlinked Discord member",
            "discord_member": discord_member.get("username")
        }
        ...
        raise HTTPException(status_code=500, detail=f"Unlink error: {str(e)}")
