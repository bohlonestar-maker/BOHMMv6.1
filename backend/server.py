...
# Voice chat endpoints (admin only)
@api_router.post("/voice/room", response_model=VoiceRoomResponse)
async def get_or_create_voice_room(current_user: dict = Depends(verify_admin)):
    """Get or create the admin voice room"""
    room_name = "admin-voice-room"
...
@api_router.post("/voice/token", response_model=MeetingTokenResponse)
async def generate_meeting_token(
    request: MeetingTokenRequest,
    current_user: dict = Depends(verify_admin)
):
    """Generate a meeting token for voice room access"""
    async with httpx.AsyncClient() as client:
        headers = {
            "Authorization": f"Bearer {DAILY_API_KEY}",
            "Content-Type": "application/json"
        ...
        room_response = await client.get(
            f"{DAILY_API_URL}/rooms/{request.room_name}",
            headers=headers
        )
        
        room_url = room_response.json().get("url", "") if room_response.status_code == 200 else ""
