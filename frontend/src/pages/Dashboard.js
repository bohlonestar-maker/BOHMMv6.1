...
import { LogOut, Plus, Pencil, Trash2, Download, Users, Mail, Phone, MapPin, MessageCircle, Clock, LifeBuoy, FileText, Calendar, Star, DollarSign, Headphones, Settings, Menu, Key, Database } from "lucide-react";
...
export default function Dashboard({ onLogout, userRole, userPermissions, userChapter, userTitle }) {
  ...
  const handleEdit = (member) => {
    ...
    setFormData({
      ...
      experience_start: member.experience_start || "",
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          <Checkbox
                            id="name_private"
                            checked={formData.name_private}
                            onCheckedChange={(checked) =>
                              setFormData({ ...formData, name_private: checked })
                            }
                            className="border-slate-400 data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600"
                          />
                          <label htmlFor="name_private" className="text-sm font-medium cursor-pointer text-slate-300">
                            Make name private (visible only to National members & chapter officers)
                          </label>
                        </div>
                      </div>
