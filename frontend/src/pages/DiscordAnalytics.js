...
export default function DiscordAnalytics() {
  ...
  const formatLastActivity = (dateStr) => {
    ...
    // Return relative time with actual date
    if (diffMins < 1) return `Just now (${formattedDate})`;
    if (diffMins < 60) return `${diffMins}m ago (${formattedDate})`;
    if (diffHours < 24) return `${diffHours}h ago (${formattedDate})`;
    if (diffDays < 7) return `${diffDays}d ago (${formattedDate})`;
